...
<div class="container">
    ...
    <table class="table table-bordered table-hover">
        ...
        <tbody>
        @foreach(getDataByType($listResult, \App\Helpers\Business::PRICE_BY_TH1, 8) as $item)
            <tr>
    ...
    <table class="table table-bordered table-hover">
        ...
        <tbody>
        @foreach(getDataByType($listResult, \App\Helpers\Business::PRICE_BY_TH1, 7) as $item)
            <tr>
                <td>{{ $listTypeCar[$item->type_car] }}</td>
                <td class="price">{{$item->min_value}}</td>
                <td><span class="price">{{$item->increase_value}}</span> x {{$item->increase}} KG</td>
    ...
    <table class="table table-bordered table-hover">
        ...
        @foreach(getDataByType($listResult, \App\Helpers\Business::PRICE_BY_TH2) as $item)
            <tr>
                <td>{{ $item->fromProvince->name }}</td>
                <td>{{$item->toProvince->name}}</td>
                <td class="price">{{$item->min_value}}</td>
            </tr>
        @endforeach
        </tbody>
    ...
    <hr />
    <h4 class="text-center">GIAO HÀNG NGOẠI TỈNH BÌNH DƯƠNG</h4>
    <table class="table table-bordered table-hover">
        <thead>
        <tr>
            <th colspan="2" class="text-center">HÀNH TRÌNH</th>
            <th rowspan="2" class="text-center">TRỌNG LƯỢNG TỐI ĐA (KG)</th>
            <th rowspan="2" class="text-center">GIÁ CƯỚC</th>
            <th rowspan="2" class="text-center">CƯỚC PHÍ THÊM</th>
        </tr>
        <tr>
            <td>TỪ</td>
            <td>ĐẾN</td>
